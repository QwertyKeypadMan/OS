/* ============================================================================
 * KayaOS - Dosya Yöneticisi (File Manager)
 * ============================================================================ */
...
#define ROW_HEIGHT       40   /* 32x32 ikonlar için ideal satır yüksekliği */
...
void fm_draw_toolbar(int cx, int cy, int cw) {
    ...
    graphics_fill_rect(cx, cy, cw, TOOLBAR_HEIGHT, bg_col);

    /* Geri ve Üst Dizin Butonları */
...
void fm_draw_sidebar(int cx, int cy, int ch) {
    ...
    graphics_fill_rect(cx, cy, SIDEBAR_WIDTH, ch, bg_col);
    ...
    for (int i = 0; i < SIDEBAR_COUNT; i++) {
        ...
        if (g_fm.selected_sidebar == i) {
            graphics_fill_rect(cx + 4, item_y, SIDEBAR_WIDTH - 8, 36, current_bg);
        }
    ...
    /* Sol / Sağ Panel Ayırıcı Çizgi */
    graphics_fill_rect(cx + SIDEBAR_WIDTH - 1, cy, 1, ch, border_col);
}
...
void fm_draw_file_list(int cx, int cy, int cw, int ch) {
    ...
    if (end_index > g_fm.item_count) {
        end_index = g_fm.item_count;
    }
...
void fm_draw_statusbar(int cx, int cy, int cw, int ch) {
    ...
    graphics_fill_rect(cx, cy, cw, STATUSBAR_HEIGHT, bg_col);
...
void fm_draw(gui_window_t *win, int cx, int cy, int cw, int ch) {
    ...
    /* Bileşenleri Çiz */
    int content_h = ch - TOOLBAR_HEIGHT - STATUSBAR_HEIGHT;
    
    fm_draw_toolbar(cx, cy, cw);
    fm_draw_sidebar(cx, cy + TOOLBAR_HEIGHT, content_h);
    fm_draw_file_list(cx + SIDEBAR_WIDTH, cy + TOOLBAR_HEIGHT, cw - SIDEBAR_WIDTH, content_h);
    fm_draw_statusbar(cx, cy + ch - STATUSBAR_HEIGHT, cw, STATUSBAR_HEIGHT);
}

void fm_handle_mouse(int win_id, int mouse_x, int mouse_y, uint8_t buttons) {
    if (win_id != g_fm.win_id || !(buttons & 0x01)) return;

    /* 1. Araç Çubuğu (Toolbar) Etkileşimi */
    if (mouse_y >= 0 && mouse_y < TOOLBAR_HEIGHT) {
        if (mouse_x >= 34 && mouse_x <= 58) {
            ...
            if (!k_streq(g_fm.cwd_path, "/")) {
                ...
                if (last_slash > 0) {
                    g_fm.cwd_path[last_slash] = '\0';
                } else {
                    k_strlcpy(g_fm.cwd_path, "/", sizeof(g_fm.cwd_path));
                }
    ...
    int content_bottom = 360 - STATUSBAR_HEIGHT;

    /* 2. Sol Panel (Sidebar) Etkileşimi */
    if (mouse_x >= 0 && mouse_x < SIDEBAR_WIDTH && mouse_y >= content_top && mouse_y < content_bottom) {
        int idx = (mouse_y - content_top - 6) / ROW_HEIGHT;
    ...
    /* 3. Sağ Panel (Dosya Listesi) Etkileşimi */
    if (mouse_x >= SIDEBAR_WIDTH && mouse_y >= content_top && mouse_y < content_bottom) {
        int clicked_row = (mouse_y - content_top - 6) / ROW_HEIGHT + g_fm.scroll_offset;
...
void fm_update(void) {
    /* İhtiyaç halinde periyodik güncelleme mantığı */
}

/* ============================================================================
 * UYGULAMA YAŞAM DÖNGÜSÜ (OPEN / CLOSE)
 * ============================================================================ */
...
void fm_open(void) {
    ...
    /* Sol Panel Hızlı Erişim Tanımları */
    k_strlcpy(g_fm.sidebar[0].label, "Kok Dizin", sizeof(g_fm.sidebar[0].label));
    k_strlcpy(g_fm.sidebar[0].path, "/", sizeof(g_fm.sidebar[0].path));

    k_strlcpy(g_fm.sidebar[1].label, "Masaustu", sizeof(g_fm.sidebar[1].label));
    k_strlcpy(g_fm.sidebar[1].path, "/desktop", sizeof(g_fm.sidebar[1].path));

    k_strlcpy(g_fm.sidebar[2].label, "Belgeler", sizeof(g_fm.sidebar[2].label));
    k_strlcpy(g_fm.sidebar[2].path, "/documents", sizeof(g_fm.sidebar[2].path));

    k_strlcpy(g_fm.sidebar[3].label, "Sistem", sizeof(g_fm.sidebar[3].label));
    k_strlcpy(g_fm.sidebar[3].path, "/system", sizeof(g_fm.sidebar[3].path));

    /* Başlangıç Dizini (Root) */
