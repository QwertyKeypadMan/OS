...
static void kterm_execute_command(void) {
    ...
    int i = 0;
    while (g_term.input_buf[i] == ' ') i++;
    int c = 0;
    while (g_term.input_buf[i] != '\0' && g_term.input_buf[i] != ' ' && c < 31) {
        cmd[c++] = g_term.input_buf[i++];
    }
    cmd[c] = '\0';

    while (g_term.input_buf[i] == ' ') i++;
    int a = 0;
    while (g_term.input_buf[i] != '\0' && a < 95) {
        arg[a++] = g_term.input_buf[i++];
    }
    arg[a] = '\0';

    /* KOMUT MANTIĞI */
    if (k_streq(cmd, "help")) {
        kterm_append_line("Komutlar: help clear pwd cd ls mkdir touch exit", col_info);
        ...
        /* 2. Bir Üst Dizin (cd ..) */
            ...
            if (!k_streq(g_term.cwd_path, "/")) {
                ...
                if (parent_node >= 0) {
                    g_term.current_dir_node = parent_node;
                } else {
                    g_term.current_dir_node = ramfs_root();
                }
            }
        } 
        /* 3. Belirli Klasöre Geçiş (cd <klasor_adi> veya cd /desktop) */
        else {
        ...
        int dir = g_term.current_dir_node;
            ...
            while (child >= 0) {
                ...
                if (cnode) {
                    uint32_t item_color = (cnode->type == RAMFS_NODE_DIR) 
                                           ? graphics_rgb(120, 200, 255)  /* Klasörler Mavi */
                                           : graphics_rgb(220, 220, 220); /* Dosyalar Beyaz */
...
void kterm_draw_cb(gui_window_t *win, int cx, int cy, int cw, int ch) {
    ...
    for (int i = 0; i < TERM_ROWS; i++) {
        if (g_term.lines[i].text[0] != '\0') {
            graphics_draw_text(cx + margin_x, current_y, g_term.lines[i].text, g_term.lines[i].color, bg_color, 1);
    ...
    graphics_draw_text(cx + margin_x, current_y, prompt_str, prompt_color, bg_color, 1);

    int input_x = cx + margin_x + (prompt_len * 8);
    if (g_term.input_len > 0) {
        graphics_draw_text(input_x, current_y, g_term.input_buf, input_color, bg_color, 1);
    }
